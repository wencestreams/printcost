...
const exportInvoice = (order, filaments, extras, settings, invoiceNumber) => {
  ...
  const L = czech ? {
    ...
    subtotal:"Mezisoučet",total:"Celkem k úhradě",notice:"Nejsem plátce DPH.",
    printTime:(m)=>`Čas tisku (${m} min)`,elec:"Elektřina",footer:`Vystaveno pomocí PrintCost v${VERSION}`,
  } : {
    ...
    subtotal:"Subtotal",total:"Total Due",notice:"Not a VAT payer.",
    printTime:(m)=>`Print time (${m} min)`,elec:"Electricity",footer:`Generated by PrintCost v${VERSION}`,
  };
...
function SettingsTab({ settings, setSettings, filaments, setFilaments, models, setModels, extras, setExtras, orders, setOrders, accent, syncKey, syncStatus, onConnect, onDisconnect, onGenerate }) {
  ...
  return (
    <div style={{maxWidth:600}}>
      ...
      <div style={{textAlign:"center",fontSize:11,color:"#333",marginTop:16}}>PrintCost v{VERSION}</div>
    </div>
  );
}
